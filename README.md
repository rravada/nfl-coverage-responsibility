...
* **1. The Dynamic Matchup Matrix:** A live, updating UI grid displaying the exact probability stream of every defender's coverage responsibility at that specific millisecond. It captures coverage adjustments in real time, showing a safety fluidly transition from a deep zone landmark to a tight man-to-man assignment as a receiver crosses his threshold.
* **2. The Spatial Disguise Index (SDI):** A proprietary metric that quantifies defensive deception on a 0–100 scale. By extracting the multi-class TreeSHAP value slice for the zone-drop class index, the system isolates marginal attributions of field-ownership geometry to highlight high-IQ secondary players who successfully masked their coverage intentions to bait quarterbacks.
* **3. The Real-Time Breakdown Predictor ("Burn Risk"):** A fully vectorized engine driven by Pandas `groupby + shift` operations that continuously monitors coverage integrity. The moment a defender drifts outside a zone landmark or a matchup cushion collapses, the UI flags a localized "Burn Risk" failure frame. Simultaneously, the SHAP engine extracts the model weights for that exact frame, translating them into human-readable coaching notes (e.g., *"Safety bit on play-action, sacrificing 4.2 yards of vertical leverage"*).

---

## Project Structure
* `data/raw/` — Ingested Next Gen Stats tracking datasets, play metadata, and PFF scouting files.
* `src/` — Modular production engineering library handling spatial normalizations, feature matrix assembly, and geometric boundary calculations.
* `pipelines/` — Production-grade automation scripts:
    * `train.py` — Handles matrix assembly, class imbalance mitigation, data type downcasting, and multi-class model training.
    * `evaluate.py` — Runs self-contained, native C++ `DMatrix` inference, extracts TreeSHAP tensors, and persists tabular deliverables.
